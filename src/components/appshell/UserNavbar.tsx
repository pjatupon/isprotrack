...
import { Search, Bell, User, LogOut, ChevronDown, Menu, Settings } from "lucide-react";
import Link from "next/link";
import Image from "next/image";
import { useRouter } from "next/navigation";
import { useTransition } from "react";
import { authClient } from "@/lib/auth-client";
import { roleLabel, type UserRole } from "./nav";
...
export function UserNavbar({ user, onMenuToggle }: UserNavbarProps) {
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 w-full items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-6 shadow-xs">
      {/* Left Zone: Mobile menu button + Brand Logo & Title */}
      <div className="flex items-center gap-3">
        ...
        <Link href="/" className="flex items-center gap-2.5 group transition-transform hover:scale-[1.01]">
          <Image
            priority
        ...
        <Dropdown>
          <Dropdown.Trigger className="flex items-center gap-2.5 rounded-full p-1 pr-2.5 hover:bg-slate-100 transition focus:outline-none cursor-pointer">
            <Avatar className="h-8 w-8 bg-[#8B0000] text-white font-bold text-xs">
              <Avatar.Fallback className="bg-[#8B0000] text-white">{initial}</Avatar.Fallback>
            </Avatar>
            <div className="hidden sm:block text-left leading-tight">
              <p className="text-xs font-semibold text-slate-800">{user.name}</p>
              <p className="text-[0.68rem] text-slate-500">{roleLabel[user.role]}</p>
            </div>
